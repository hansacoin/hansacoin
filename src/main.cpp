...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1)
        nSubsidy = 677500 * COIN; // Proof of Job mintage. 677,500 coins (around 2%) for Proof of Jobs rewards.
    else
    if (nHeight <= 100) //Pow block
        nSubsidy = 0 * COIN; // Zero blocks start.
	else
    if (nHeight < 5000) //Pow block
        nSubsidy = 200 * COIN; // Kick start Pow block reward for early miners.
    else
    if (nHeight < 10000) //Pow block
        nSubsidy = 100 * COIN; // Pow block reward
    else
    if (nHeight < 30000) //Pow block
        nSubsidy = 80 * COIN; // Pow block reward
    else
    if (nHeight < 50000) //Pow block
        nSubsidy = 70 * COIN; // Pow block reward		
    else
    if (nHeight < 70000) //Pow block
        nSubsidy = 60 * COIN; // Pow block reward
    else
    if (nHeight < 100000) //Pow block
        nSubsidy = 50 * COIN; // Pow block reward		
    else
    if (nHeight < LAST_POW_BLOCK) //Pow block
        nSubsidy = 20 * COIN; // Pow block reward	
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8); // Stable 10% annual stake + transaction fees.
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "hansa", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        const char* pszTimestamp = "X freedom";
        CTransaction txNew;
        txNew.nTime = 1417695597;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = !fTestNet ? 136855 : 136855;
       ...
       printf("block.nNonce = %u \n", block.nNonce);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x6918e474ef871f2d06bb6af50ed675929dcce915d33d0fe07ef6ba6abb10121a"));
        block.print();
